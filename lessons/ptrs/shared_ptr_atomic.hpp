 ...
 */
	...
	template <typename _Tp, _Lock_policy _Lp>
	_GLIBCXX20_DEPRECATED_SUGGEST("std::atomic<std::shared_ptr<T>>")
	inline __shared_ptr<_Tp, _Lp> atomic_exchange(
	    __shared_ptr<_Tp, _Lp>* __p,
	    __shared_ptr<_Tp, _Lp> __r
	) {
		return std::atomic_exchange_explicit(__p, std::move(__r), memory_order_seq_cst);
	...
	/// @} group pointer_abstractions
		...
		class _Sp_atomic {
			...
			struct _Atomic_count {
				...
				pointer lock(memory_order __o) const noexcept {
					...
					while (!__aref.compare_exchange_strong(
					    __current,
		...
		#if __glibcxx_atomic_wait
					// Precondition: caller holds lock!
					void _M_wait_unlock(const element_type* const& __ptr, memory_order __o) const noexcept {
						auto __old_ptr = __ptr;
						_GLIBCXX_TSAN_MUTEX_PRE_UNLOCK(&_M_val);
						uintptr_t __old_pi = _AtomicRef(&_M_val).fetch_sub(1, memory_order_relaxed) - 1u;
						_GLIBCXX_TSAN_MUTEX_POST_UNLOCK(&_M_val);
			...
			static typename _Atomic_count::pointer _S_add_ref(typename _Atomic_count::pointer __p) {
				if (__p) {
					if constexpr (__is_shared_ptr<_Tp>)
						__p->_M_add_ref_copy();
					else
		...
		public:
			...
			operator shared_ptr<_Tp>() const noexcept { return _M_impl.load(memory_order_seq_cst); }

			void store(shared_ptr<_Tp> __desired, memory_order __o = memory_order_seq_cst) noexcept {
			...
			shared_ptr<_Tp>
			exchange(shared_ptr<_Tp> __desired, memory_order __o = memory_order_seq_cst) noexcept {
				_M_impl.swap(__desired, __o);
				return __desired;
			}

			bool compare_exchange_strong(
			    shared_ptr<_Tp>& __expected,
		...
		#if __glibcxx_atomic_wait
				void wait(value_type __old, memory_order __o = memory_order_seq_cst) const noexcept {
		...
		public:
			...
			operator weak_ptr<_Tp>() const noexcept { return _M_impl.load(memory_order_seq_cst); }

			void store(weak_ptr<_Tp> __desired, memory_order __o = memory_order_seq_cst) noexcept {
				_M_impl.swap(__desired, __o);
			}
			...
			weak_ptr<_Tp>
			exchange(weak_ptr<_Tp> __desired, memory_order __o = memory_order_seq_cst) noexcept {
				_M_impl.swap(__desired, __o);
				return __desired;
			}

			bool compare_exchange_strong(
			    weak_ptr<_Tp>& __expected,
