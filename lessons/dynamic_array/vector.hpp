 ...
 */
	...
	public:
		...
		_GLIBCXX20_CONSTEXPR
		_Tp_alloc_type& _M_get_Tp_allocator() _GLIBCXX_NOEXCEPT { return this->_M_impl; }
		...
		static _GLIBCXX20_CONSTEXPR size_t _S_max_size(const _Tp_alloc_type& __a) _GLIBCXX_NOEXCEPT {
			// std::distance(begin(), end()) cannot be greater than PTRDIFF_MAX,
			// and realistically we can't store more than PTRDIFF_MAX/sizeof(T)
			// (even if std::allocator_traits::max_size says we can).
			const size_t __diffmax = __gnu_cxx::__numeric_traits<ptrdiff_t>::__max / sizeof(_Tp);
			const size_t __allocmax = __gnu_cxx::__alloc_traits<_Alloc>::max_size(__a);
	...
	public:
		...
		_GLIBCXX20_CONSTEXPR
		_Alloc_result _M_allocate_at_least(size_t __n) {
			typedef __gnu_cxx::__alloc_traits<_Tp_alloc_type> _Tr;
			_Alloc_result __r;
	...
	protected:
		...
		void _M_create_storage(size_t __n) {
			...
			constexpr void _M_append_range_to(_Rg&& __rg, pointer& __ptr) {
				__ptr = std::__uninitialized_copy_a(
				    ranges::begin(__rg),
				    ranges::end(__rg),
				    __ptr,
				    _M_get_Tp_allocator()
	...
	private:
			...
			static _GLIBCXX20_CONSTEXPR pointer _S_relocate(
			    pointer __first,
			    pointer __last,
			    pointer __result,
	...
	public:
		...
		public:
			...
			_GLIBCXX20_CONSTEXPR
			vector(vector&& __rv, const __type_identity_t<allocator_type>& __m) noexcept(noexcept(vector(
			    std::declval<vector&&>(),
		 ...
		 */
		...
		#if __glibcxx_concepts  // C++ >= C++20
					if constexpr (
					    sized_sentinel_for<_InputIterator, _InputIterator> || forward_iterator<_InputIterator>
					) {
						const auto __n = static_cast<size_type>(ranges::distance(__first, __last));
			...
			constexpr vector(from_range_t, _Rg&& __rg, const _Alloc& __a = _Alloc()) : vector(__a) {
		 ...
		 */
			...
			constexpr void assign_range(_Rg&& __rg) {
				...
				if constexpr (ranges::forward_range<_Rg> || ranges::sized_range<_Rg>) {
					...
					reserve(__n);
					auto __first = ranges::copy_n(ranges::begin(__rg), size(), this->_M_impl._M_start).in;
					[[maybe_unused]] const auto __diff = __n - size();
					_GLIBCXX_ASAN_ANNOTATE_GROW(__diff);
					_Base::_M_append_range(ranges::subrange(std::move(__first), ranges::end(__rg)));
		...
		void pop_back() _GLIBCXX_NOEXCEPT {
			...
			constexpr void append_range(_Rg&& __rg) {
				...
				if constexpr (ranges::forward_range<_Rg> || ranges::sized_range<_Rg>) {
					...
					std::__uninitialized_copy_a(ranges::begin(__rg), ranges::end(__rg), __mid, __a);
					...
					if constexpr (_S_use_relocate())
						...
						struct _Guard_elts {
							...
							constexpr ~_Guard_elts() { std::_Destroy(_M_first, _M_last, _M_alloc); }
	...
	protected:
		...
		_GLIBCXX20_CONSTEXPR _Alloc_result
		...
		_GLIBCXX20_CONSTEXPR void _M_replace_storage(pointer __start, pointer __end, size_type __cap) {
		...
		_M_replace_with(size_type __n, _ForwardIterator __first, _ForwardIterator __last) {
		...
		// Called by the second initialize_dispatch above
		template <typename _ForwardIterator>
		_GLIBCXX20_CONSTEXPR void _M_range_initialize(
		...
		_M_range_initialize_n(_Iterator __first, _Sentinel __last, size_type __n) {
			_Alloc_result __r =
			    this->_M_allocate_at_least(_S_check_init_len(__n, _M_get_Tp_allocator()));
		...
		// Called by the range insert to implement [23.1.1]/9
		template <typename _InputIterator>
		_GLIBCXX20_CONSTEXPR void _M_insert_dispatch(
		    iterator __pos,
		    _InputIterator __first,
		    _InputIterator __last,
		    __false_type
		...
		template <typename _InputIterator>
		_GLIBCXX20_CONSTEXPR void _M_range_insert(
		    iterator __pos,
		    _InputIterator __first,
		    _InputIterator __last,
		...
		void _M_fill_append(size_type __n, const value_type& __x);
			...
			struct _Temporary_value {
				...
				_GLIBCXX20_CONSTEXPR value_type& _M_val() noexcept { return _M_storage._M_val; }

			private:
				_GLIBCXX20_CONSTEXPR _Tp* _M_ptr() noexcept { return std::__addressof(_M_storage._M_val); }
	...
	#else
			template <typename _Ptr>
			value_type* _M_data_ptr(_Ptr __ptr) const {
		...
		#if __glibcxx_containers_ranges  // C++ >= 23
			template <ranges::input_range _Rg, __allocator_like _Alloc = allocator<ranges::range_value_t<_Rg>>>
			vector(from_range_t, _Rg&&, _Alloc = _Alloc()) -> vector<ranges::range_value_t<_Rg>, _Alloc>;
		#endif
	#endif
	...
	/// See std::vector::swap().
	template <typename _Tp, typename _Alloc>
	_GLIBCXX20_CONSTEXPR inline void swap(
