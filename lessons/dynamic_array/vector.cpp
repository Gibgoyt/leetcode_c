 ...
 */
	...
	_GLIBCXX20_CONSTEXPR void vector<_Tp, _Alloc>::reserve(size_type __n) {
		...
		if (this->capacity() < __n) {
			...
			_M_replace_storage(__tmp.__ptr, __tmp.__ptr + __old_size, __tmp.__count);
		}
	}
	...
	template <typename _Tp, typename _Alloc>
	_GLIBCXX20_CONSTEXPR typename vector<_Tp, _Alloc>::iterator vector<_Tp, _Alloc>::_M_erase(
	    iterator __position
	) {
		if (__position + 1 != end())
	...
	template <typename _Tp, typename _Alloc>
	_GLIBCXX20_CONSTEXPR vector<_Tp, _Alloc>& vector<_Tp, _Alloc>::operator=(
...

		{
	...
	#endif
			{
				...
				struct _Guard_elts {
					...
					_GLIBCXX20_CONSTEXPR
					~_Guard_elts() { std::_Destroy(_M_first, _M_last, _M_alloc); }
...

		{
			...
			// If this throws, the existing elements are unchanged.
				...
				if constexpr (_S_use_relocate()) {
					// Relocation cannot throw.
					__new_finish =
					    std::__relocate_a(__old_start, __old_finish, __new_start, _M_get_Tp_allocator());
					++__new_finish;
				} else
	#endif
			{
	...
	vector<_Tp, _Alloc>::_M_fill_insert(iterator __position, size_type __n, const value_type& __x) {
		if (__n != 0) {
			if (__position.base() == this->_M_impl._M_finish)
			...
			else if (size_type(this->_M_impl._M_end_of_storage - this->_M_impl._M_finish) >= __n) {
	#if __cplusplus < 201103L
				...
				const size_type __len1 = _M_check_len(__n, "vector::_M_fill_insert");
	...
	template <typename _Tp, typename _Alloc>
	_GLIBCXX20_CONSTEXPR void
	vector<_Tp, _Alloc>::_M_fill_append(size_type __n, const value_type& __x) {
		if (size_type(this->_M_impl._M_end_of_storage - this->_M_impl._M_finish) >= __n) {
			_GLIBCXX_ASAN_ANNOTATE_GROW(__n);
			this->_M_impl._M_finish =
			    std::__uninitialized_fill_n_a(this->_M_impl._M_finish, __n, __x, _M_get_Tp_allocator());
			_GLIBCXX_ASAN_ANNOTATE_GREW(__n);
		} else {
			// Make local copies of these members because the compiler thinks
			// the allocator can alter them if 'this' is globally reachable.
			...
			__catch(...) {
				std::_Destroy(__new_start + __old_size, __new_finish, _M_get_Tp_allocator());
				_M_deallocate(__new_start, __len);
				__throw_exception_again;
			}
			std::_Destroy(__old_start, __old_finish, _M_get_Tp_allocator());
		...
		_GLIBCXX20_CONSTEXPR void vector<_Tp, _Alloc>::_M_default_append(size_type __n) {
			if (__n != 0) {
				...
				if (__navail >= __n) {
					...
					const size_type __len1 = _M_check_len(__n, "vector::_M_default_append");
					_Alloc_result __r = this->_M_allocate_at_least(__len1);
					const size_type __len = __r.__count;
					pointer __new_start(__r.__ptr);
	...
	template <typename _Tp, typename _Alloc>
	template <typename _InputIterator>
	_GLIBCXX20_CONSTEXPR void vector<_Tp, _Alloc>::_M_range_insert(
	    iterator __pos,
	    _InputIterator __first,
	    _InputIterator __last,
	    std::input_iterator_tag
	...
	_GLIBCXX20_CONSTEXPR void vector<_Tp, _Alloc>::_M_range_insert(
		...
		if (__first != __last) {
			...
			if (size_type(this->_M_impl._M_end_of_storage - this->_M_impl._M_finish) >= __n) {
				...
				const size_type __ask = _M_check_len(__n, "vector::_M_range_insert");
	#if __cplusplus < 201103L
		...
		constexpr auto vector<_Tp, _Alloc>::insert_range(const_iterator __pos, _Rg&& __rg) -> iterator {
			if (__pos == cend()) {
			...
			if constexpr (ranges::forward_range<_Rg>) {
				...
				{
					const size_type __ask = _M_check_len(__n, "vector::insert_range");
					...
					struct _Guard : _Guard_alloc {
						...
						constexpr ~_Guard() {
							std::_Destroy(this->_M_storage, _M_finish, this->_M_vect._M_get_Tp_allocator());
						}
					};
					...
					__guard._M_finish =
					    std::__uninitialized_move_if_noexcept_a(__old_start, __ins, __new_start, __alloc);
					...
					__guard._M_finish = std::__uninitialized_move_if_noexcept_a(
					    __ins,
					    __old_finish,
					    __guard._M_finish,
					    __alloc
				...
				return insert_range(
				    __pos,
				    vector(from_range, std::forward<_Rg>(__rg), _M_get_Tp_allocator())
				);
		}
	...
	vector<bool, _Alloc>::_M_fill_insert(iterator __position, size_type __n, bool __x) {
		...
		if (capacity() - size() >= __n) {
			...
			iterator __finish = std::copy(__position, __end, __i + difference_type(__n));
			this->_M_deallocate();
	...
	template <typename _Alloc>
	template <typename _ForwardIterator>
	_GLIBCXX20_CONSTEXPR void vector<bool, _Alloc>::_M_insert_range(
	    iterator __position,
	    _ForwardIterator __first,
	    _ForwardIterator __last,
	    std::forward_iterator_tag
	...
	template <typename _Alloc>
	_GLIBCXX20_CONSTEXPR typename vector<bool, _Alloc>::iterator vector<bool, _Alloc>::_M_erase(
	    iterator __position
	) {
		if (__position + 1 != end())
