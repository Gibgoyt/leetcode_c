...
namespace bustub {
	/*
	 *	all bodies are currently stub implementations, we will fill them across project 3 (i.e. sort)
	 *	and project 4 (i.e. MVCC) tasks
	 *
	 *	project 4, task 1 (timestamp allocation) does not yet require any of these implementations to be live
	 *	only touches TransactionManager and Watermark
	*/

	/*
	 *	project 3 sort utilities
	*/
	 ...
	 * 	
	 * 	must implement strict-weak ordering over SortEntry using order_bys_
	*/
	auto TupleComparator::operator()(
	...
	*/
	 ...
	 *	@return An		- optional tuple that represents the reconstructed tuple. 
	 *				  If the tuple is deleted as the result, returns
	 *	std::nullopt.
	 */
	 ...
	 *				  std::nullopt if the tuple did not exist at the
	 *	time.
	 */
	 ...
	 *	@param base_tuple	- The base tuple before the update, the one retrieved from the table heap. 
	 *				  nullptr if the tuple is
	 *	deleted.
	...
	/*
	 *	debug print:
	 *		walks every RID in the table heap and prints:
	 *			- current base tuple (with ts = txnN if tentative, or a numeric commit_ts if already committed)
	 *			- every undo log in the chain in order from newest to oldest
	 *			  including the writing txnId, the log index, modified payload
	 *			  (`_` for fields NOT modified by that log) + the ts
	 *	
	 *	indispensible for debugging, project 4 tasks 2+
