...
#
# Usage:
#   ./compile.sh --compile <source.c>
#
# Example:
#   ./compile.sh --compile humpty_dumpty.c   # produces ./humpty_dumpty
#
# Behavior:
...
usage() {
    echo "Usage: $0 --compile <source.c>" >&2
    exit 1
}
...
# Derive binary name by stripping the .c suffix.
OUT="${SRC%.c}"

# If stripping didn't change anything, the user didn't pass a .c file.
if [[ "$OUT" == "$SRC" ]]; then
    echo "Error: '$SRC' is not a .c file." >&2
    exit 1
fi
...
# the CPU-side code in our .c file.
gcc \
    "$SRC" \
    -o "$OUT"
