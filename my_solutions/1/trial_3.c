...
bool hasDuplicate(
	...
	*/
	...
	// TODO!!: exiting inside a function is probably ass, what is the best way to return error without changing bool function return type??
	if (NULL == keys) {
		fprintf(stderr, "Error, malloc() failed for `keys` faild\n");
		exit(1);
	}
	...
	// TODO!!: exiting inside a function is probably ass, what is the best way to return error without changing bool function return type??
	if (NULL == used) {
		fprintf(stderr, "Error, calloc() failed for `used`\n");
		free(keys);
		exit(1);
	}
