# Project #3 - Query Execution - CMU 15-445/645 :: Intro to Database Systems (Spring 2026)

> Source: <https://15445.courses.cs.cmu.edu/spring2026/project3/>
...
At this point in the semester, you have implemented many internal components of a database management system. In [Project #1](https://15445.courses.cs.cmu.edu/spring2026/project1), you implemented a buffer pool manager. In [Project #2](https://15445.courses.cs.cmu.edu/spring2026/project2), you implemented a b-plus tree index. In this project, you will implement the components that allow BusTub to execute queries. You will create the operator executors that execute SQL queries and implement optimizer rules to transform query plans.

This project is composed of several tasks:

-   [Task #1: Access Method Executors](#task1)
-   [Task #2: Aggregation and Join Executors](#task2)
-   [Task #3: HashJoin Executor and Optimization](#task3)
-   [Task #4: External Merge Sort + Limit Executors + Window Functions](#task4)
-   [Optional Leaderboard Task](#leaderboard)
...
[![project-structure](assets/project-structure.svg)](assets/project-structure.svg)
...
> **⚠️ WARNING:** You can also use [BusTub Web Shell](https://15445.courses.cs.cmu.edu/spring2026/bustub/) to run the examples below. It is a complete reference solution of the system running in your browser!
...
[![mock\_scan](assets/mock_scan.svg)](assets/mock_scan.svg)
...
Agg { types=["max"], aggregates=["#0.1"], group_by=["#0.0"] }
  HashJoin { type=Inner, left_key=["#0.0"], right_key=["#0.0"] }
...
[![more\_complex\_example](assets/more_complex_example.svg)](assets/more_complex_example.svg)
...
The executors are created from an execution plan in [src/execution/executor\_factory.cpp](https://github.com/cmu-db/bustub/blob/master/src/execution/executor_factory.cpp "View Source Code") .
...
-   [src/optimizer/seqscan\_as\_indexscan.cpp](https://github.com/cmu-db/bustub/blob/master/src/optimizer/seqscan_as_indexscan.cpp "View Source Code")
...
Delete { table_oid=15 } | (__bustub_internal.delete_rows:INTEGER)
  SeqScan { table=t1 } | (t1.v1:INTEGER, t1.v2:VARCHAR(100))
...
The IndexScanExecutor does point lookups, partial prefix lookups, and ordered scans using the b-plus tree index to retrieve tuples in the table. The executor should be able to support several point lookups on the same index, as well as composite keys.

You will need to implement the index scan by supporting the following three types of queries:
...
3.  Partial Prefix Lookup: For example, `SELECT FROM <table> WHERE <index col 1> = <val 1>` (where the index is on col 1, col 2).
...
Index created with id = 0 with type = BPlusTree
...
bustub> CREATE INDEX t1v1v2 ON t1(v1, v2);
Index created with id = 1 with type = BPlusTree

bustub> EXPLAIN (o,s) SELECT * FROM t1 WHERE v1 = 1 AND v2 = 2;
=== OPTIMIZER ===
IndexScan { index_oid=0, filter=(#0.0=1, #0.1=2) } | (t1.v1:INTEGER, t1.v2:INTEGER)
```

In previous versions of BusTub, indexes were hardcoded to a specific two-integer template. This is **no longer the case**. Your executor must dynamically dispatch the execution to the correct pre-compiled B+ Tree template (`GenericKey<4>, <8>, <16>, <32>, or <64>`) based on the physical size of the indexed columns at runtime. You can calculate the required key size by summing the `GetStorageSize()` of each column in the index's key schema, and then use C++ template dispatching (e.g., an if/else chain or switch statement) to execute your index scan logic. Note that our test cases will not contain duplicate keys.
...
**Hint:** Please use `ScanKey` for the point lookup, and use `Index Iterator` for the ordered scan/ partial prefix scan.
...
**Hint:** Please only optimize `SeqScan` to `IndexScan` in the 2 scenarios mentioned above (i.e. point lookup, ordered scan, and partial prefix lookup).

**Hint:** For partial prefix scans, use `BeginIterator` on a lower bound key to prevent doing an O(N) scan.

## Task #2 - Aggregation & Join Executors

You will complete your implementation in the following files:

-   [src/include/execution/plans/aggregation\_plan.h](https://github.com/cmu-db/bustub/blob/master/src/include/execution/plans/aggregation_plan.h "View Source Code")
-   [src/include/execution/executors/aggregation\_executor.h](https://github.com/cmu-db/bustub/blob/master/src/include/execution/executors/aggregation_executor.h "View Source Code")
-   [src/execution/aggregation\_executor.cpp](https://github.com/cmu-db/bustub/blob/master/src/execution/aggregation_executor.cpp "View Source Code")
-   [src/include/execution/executors/nested\_loop\_join\_executor.h](https://github.com/cmu-db/bustub/blob/master/src/include/execution/executors/nested_loop_join_executor.h "View Source Code")
-   [src/execution/nested\_loop\_join\_executor.cpp](https://github.com/cmu-db/bustub/blob/master/src/execution/nested_loop_join_executor.cpp "View Source Code")
-   [src/include/execution/executors/nested\_index\_join\_executor.h](https://github.com/cmu-db/bustub/blob/master/src/include/execution/executors/nested_index_join_executor.h "View Source Code")
-   [src/execution/nested\_index\_join\_executor.cpp](https://github.com/cmu-db/bustub/blob/master/src/execution/nested_index_join_executor.cpp "View Source Code")
...
**Hint:** You must handle NULL values in the input of the aggregation functions (i.e., a tuple may have a NULL value for the attribute used in the aggregation function). See test cases for expected behavior.

**Hint:** Group-by columns can also have NULL values. You may want to consider modifying the way aggregate keys are compared. Think about how null comparisons usually work and how it should work in this case.
...
We will provide all test cases on Gradescope AS-IS. We will not test with strange edge cases involving NULLs in indexes. At this point, you should pass [SQLLogicTests - #7 to #13](#testing).

## Task #3 - HashJoin Executor and Optimization

You will complete your implementation in the following files:

-   [src/include/storage/page/intermediate\_result\_page.h](https://github.com/cmu-db/bustub/blob/master/src/include/storage/page/intermediate_result_page.h "View Source Code")
-   [src/include/execution/executors/hash\_join\_executor.h](https://github.com/cmu-db/bustub/blob/master/src/include/execution/executors/hash_join_executor.h "View Source Code")
-   [src/execution/hash\_join\_executor.cpp](https://github.com/cmu-db/bustub/blob/master/src/execution/hash_join_executor.cpp "View Source Code")
-   [src/optimizer/nlj\_as\_hash\_join.cpp](https://github.com/cmu-db/bustub/blob/master/src/optimizer/nlj_as_hash_join.cpp "View Source Code")
...
As you may be aware, a specific page format will be needed for the intermediate partition results. In this project, you will be the one who designs the page format. Take a look at `intermediate_result_page.h` and you will find `IntermediateResultPage`, which is dedicated for storing the intermediate execution results but contains minimal pre-defined members/methods. You should design the page layout and implement the read/write methods for the page. Before designing `IntermediateResultPage`, it is recommended to first read the section on [Task 4 - External Merge Sort](#task4) to decide whether you want to use your implementation of `IntermediateResultPage` for both tasks.
...
-   [src/include/execution/executors/external\_merge\_sort\_executor.h](https://github.com/cmu-db/bustub/blob/master/src/include/execution/executors/external_merge_sort_executor.h "View Source Code")
-   [src/execution/external\_merge\_sort\_executor.cpp](https://github.com/cmu-db/bustub/blob/master/src/execution/external_merge_sort_executor.cpp "View Source Code")
-   [src/include/execution/executors/limit\_executor.h](https://github.com/cmu-db/bustub/blob/master/src/include/execution/executors/limit_executor.h "View Source Code")
-   [src/execution/limit\_executor.cpp](https://github.com/cmu-db/bustub/blob/master/src/execution/limit_executor.cpp "View Source Code")
-   [src/include/execution/executors/window\_function\_executor.h](https://github.com/cmu-db/bustub/blob/master/src/include/execution/executors/window_function_executor.h "View Source Code")
-   [src/execution/window\_function\_executor.cpp](https://github.com/cmu-db/bustub/blob/master/src/execution/window_function_executor.cpp "View Source Code")
...
**Hint:** To better understand what to do and where to start for `IntermediateResultPage`, you can take a look at the layout of the index pages you worked on in [Project #2](https://15445.courses.cs.cmu.edu/spring2026/project2) (e.g. a `char` array can be used as the start of page data). It would be helpful to think of how your executor will call the read/write methods.
...
[![window\_function\_execution\_model](assets/window_function_execution_model.jpg)](assets/window_function_execution_model.jpg)

Consider the following examples using this example table below:
...
The below example calculates a moving average of the salary for each department. You can consider it as first sort the rows for each partition by name and then calculate the average of the row before the current row, current row and the row after current row.
...
The query below calculates a moving average of the salary for each department. Different from previous example, when window frames are omitted and order by clauses not omitted, it calculates from the first row to the current row for each partition.
...
This query show that when order by and window frames are both omitted, it calculates from the first row to the last row for each partition, which means the results within the partition should be the same.
...
### Query 3: What Am I Doing With My Life?

After all the hard work with Q1 and Q2, here's something short and sweet. Assume you have the following two tables representing everyone that is under the age of 16, and everyone who are currently in college. Someone might try to construct a query that outputs the intersection of the two tables to discover child prodigies. You know the result will have few matches, but your join executors does not know this ahead of time. Thus, what can you do to help speed up the process?
...
-   Your optimization should not affect correctness. You can optimize for specific cases, but it should work for all inputs in your optimized cases.
    -   Allowed: only handling 3-table join reordering.
...
In this project, we use your implementation of b-plus tree index from [Project #2](https://15445.courses.cs.cmu.edu/spring2026/project2) as the underlying data structure for all index operations. Therefore, successful completion of this project relies on a working implementation of the b-plus tree.
